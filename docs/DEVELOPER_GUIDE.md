...
**`applyTicketPermissions` ne retire jamais l'accès d'un membre, et aligne les rôles sur la catégorie** — règle valable pour les deux cas d'`acquireChannel` (première acquisition comme mise à jour). Trois conséquences dans le code, à ne pas « simplifier » :
...
La méthode retourne `{ removedRoleIds }`, qu'`acquireChannel` affiche dans la réponse éphémère et dans le log. `buildTicketPermissionOverwrites` sert aux deux chemins — tel quel à `guild.channels.create` (ouverture par le panel, où le salon est neuf et le refus `@everyone` indispensable), filtré à l'acquisition.
